export default async function handler(req, res) {
  ...
  try {
    const { action, appId, appSecret } = req.body;
    ...
    if (!tokenResponse.ok) {
      ...
      return res.status(401).json({ 
        success: false, 
        message: 'Tiki Authentication Failed. Vui lòng kiểm tra lại App ID, App Secret và chắc chắn bạn đã Chấp nhận kết nối trên Tiki Seller Center.',
        details: errorData
      });
    }
