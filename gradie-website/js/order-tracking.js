...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function doTrack(orderNo) {
    ...
    if (!order) {
      resultDiv.innerHTML = '<p style="color:red; text-align:center; font-weight:500; margin-top:20px;">Không tìm thấy đơn hàng. Vui lòng kiểm tra lại mã tra cứu.</p>';
    } else {
      ...
      resultDiv.innerHTML = `
        <div style="padding:25px; border:1px solid var(--border-gold); background:#ffffff; border-radius:12px; margin-top:25px; box-shadow:0 8px 25px rgba(0,0,0,0.02);">
          <div style="border-bottom:1px solid #f0eeeb; padding-bottom:12px; margin-bottom:15px;">
            <h3 style="font-family:'Playfair Display', serif; font-size:1.3rem; margin:0; color:#1a1a1a;">Chi Tiết Đơn Hàng</h3>
            <span style="font-size:0.85rem; color:#888;">Mã đơn hàng: ${order.orderNumber}</span>
          </div>
          
          <div style="display:grid; grid-template-columns:1fr 1fr; gap:15px; margin-bottom:20px; font-size:0.9rem; line-height:1.5;">
            <div>
              <div style="color:#888;">Người nhận:</div>
            ...
            <div style="grid-column:1/-1;">
              <div style="color:#888;">Địa chỉ giao hàng:</div>
              <span style="color:#1a1a1a;">${order.shippingAddress}</span>
            </div>
            <div style="grid-column:1/-1;">
              <div style="color:#888;">Chi tiết sản phẩm:</div>
              ${itemsList}
            </div>
          </div>
          ...
          <h4 style="font-family:'Playfair Display', serif; font-size:1.1rem; margin-bottom:15px; color:#1a1a1a;">Lịch Trình Vận Chuyển</h4>
          <div style="display:flex; flex-direction:column; gap:15px; padding:15px; background:#faf8f5; border-radius:8px;">
            <div style="display:flex; align-items:center; gap:10px;">
              ...
              <div>
                <strong style="font-size:0.9rem; color:#1a1a1a;">Đơn Hàng Đã Xác Nhận & Xác Thực COD</strong>
                <div style="font-size:0.75rem; color:#666;">Chúng tôi đã xác nhận thành công thông tin đặt hàng của bạn.</div>
              </div>
            </div>
            <div style="display:flex; align-items:center; gap:10px;">
              ...
              <div>
                <strong style="font-size:0.9rem; color:${order.status === 'Shipped' || order.status === 'Delivered' ? '#1a1a1a' : '#888'};">Đang Vận Chuyển</strong>
                <div style="font-size:0.75rem; color:#666;">Món quà thiết kế riêng của bạn đang được vận chuyển bởi đối tác giao hàng cao cấp.</div>
              </div>
            </div>
            <div style="display:flex; align-items:center; gap:10px;">
              <span style="font-size:1.2rem; color:${order.status === 'Delivered' ? '#137333' : '#b0b0b0'};">${order.status === 'Delivered' ? '🟢' : '⚪'}</span>
              <div>
                <strong style="font-size:0.9rem; color:${order.status === 'Delivered' ? '#1a1a1a' : '#888'};">Giao Hàng Thành Công</strong>
                <div style="font-size:0.75rem; color:#666;">Đơn hàng đã được thanh toán và giao tận tay người nhận. Hãy tận hưởng khoảnh khắc tuyệt vời cùng Gradie!</div>
              </div>
            </div>
          </div>
        </div>
