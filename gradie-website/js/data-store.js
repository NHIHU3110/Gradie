...
window.GradieStore = {
  ...
  normalizeProduct: function (p) {
    ...
    const sanitizeImageUrl = (url) => {
      ...
      if (lower.includes('ui-avatars.com') || lower.includes('placeholder') || lower.includes('via.placeholder')) return '';
      return cleaned;
    };
  ...
  syncLazadaProducts: async function () {
    ...
    const isRealImage = url => {
      if (!url || typeof url !== 'string') return false;
      const u = url.trim().toLowerCase();
      return u.startsWith('http') && !u.includes('placeholder') && !u.includes('ui-avatars.com') && !u.includes('via.placeholder') && !u.includes('unsplash.com');
    };
